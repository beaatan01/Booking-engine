...
st.set_page_config(page_title="Booking Engine FINAL", layout="wide")
st.title("📊 Booking Reconciliation Engine (Auto Detect + Strict Rules)")
...
def detect_header(df_raw):
    """Find the row where real headers start"""
...
if file:
    try:
        ...
        if header_row is None:
            st.error("❌ Could not automatically find header row")
            st.stop()

        st.write(f"✅ Detected header row at index: {header_row}")
        ...
        # ✅ SHOW COLUMN MAPPING (VERY IMPORTANT)
