...
st.set_page_config(page_title="Booking Engine 2.0", layout="wide")
...
if file:
    try:
        ...
        if account_col is None:
            df["Account Name"] = "UNKNOWN ACCOUNT"
        else:
            df["Account Name"] = df[account_col]

        if close_col is None:
            df["Close Date"] = ""
        else:
            df["Close Date"] = df[close_col]
        ...
        if rgo_col is None:
            ...
            df["RGO Product"] = df[rgo_col]

        if avc_col is None:
            df["AVC"] = 0
        else:
            df["AVC"] = pd.to_numeric(df[avc_col], errors="coerce").fillna(0)
        ...
        for bu in business_units:
            data = results.get(bu, {"count":0,"total":0,"top3":[]})
        ...
        st.warning("⚠️ Could not fully process file, but app is still active.")